...
#[tokio::test]
async fn decodes_recorded_compatible() {
    ...
    let (reply, compatible) = tokio::join!(
        dbus.reply(include_str!("../records/properties.json")),
        proxy.compatible(),
    );

    reply.expect("failed to send recorded D-Bus reply");
    let compatible = compatible.expect("failed to get compatible");

    assert_eq!(compatible, "raspberrypi5");
}
